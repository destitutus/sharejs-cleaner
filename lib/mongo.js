...
Mongo.prototype.findCleanup = function () {
    ...
    return this._dbConnection().then(function (db) {
        ...
        log.debug('Connection done, search outdated in ' + self.config.collection + '_ops');
        ...
        // distinct command in mongo have a stupid limitation - 16mb 
        db.collection(self.config.collection).find({}, {_id: 1}).toArray(function (err, data) {
            var docsForCleanup = [];
            var tasks = [];
            _.each(data, function (item) {
                tasks.push(function (callback) {
                    // next step is check records in oplog with time greater then cleanup date
                    opsCollection.findOne({'name': item._id, 'm.ts': {$gt: lessDate}}, function (err, data) {
                        log.debug('check', item._id, !data);
                        if (!data) {
                            docsForCleanup.push(item._id);
                        }
                        callback(err);
                    });
                });
